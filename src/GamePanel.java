...
public class GamePanel extends JPanel implements Runnable {
                ...
                //playing state
                    ...
                    switch (code) {
                        case KeyEvent.VK_LEFT:
                            if(board.valid(tetromino, x-1, y)) tetromino.move(-1, 0);
                            break;
                        case KeyEvent.VK_RIGHT:
                            if(board.valid(tetromino, x+1, y)) tetromino.move(1, 0);
                            break;
                        case KeyEvent.VK_DOWN:
                            if(board.valid(tetromino, x, y+1)) tetromino.move(0, 1);
                            break;
                        case KeyEvent.VK_UP:
    ...
    public void update(){
        ...
        if(tetromino != null && board != null) {
            ...
            if(fallCount >= fallSpeed) {
                if(board.valid(tetromino, tetromino.getX(), tetromino.getY() + 1)) {
                    tetromino.move(0, 1);
                }
                else {
    ...
    private void drawPlayScreen(Graphics2D g2) {
        ...
        g2.drawString("HIGH SCORE: " + Math.max(score, highScore), 650, 150);
